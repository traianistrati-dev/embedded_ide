...
/// inserted after when an older file has none — in either spelling: a project
/// created before the 2026-09-20 rename keeps the old banner for good.
const AUTOGEN_LINE: &str = super::codegen::AUTOGEN_BANNER;
const LEGACY_AUTOGEN_LINE: &str = super::codegen::LEGACY_AUTOGEN_BANNER;
...
fn refresh_hal_line(head: &str, mcu_name: &str, runtime: EspRuntime) -> String {
    ...
    for line in head.split_inclusive('\n') {
        ...
        out.push_str(line);
        if !done && matches!(line.trim_end(), AUTOGEN_LINE | LEGACY_AUTOGEN_LINE) {
            out.push_str(&wanted);
            done = true;
        }
    }
...
{code}"
    ...
    fn hal_header_line_tracks_version_and_runtime() {
        ...
        let banner = updated.find(AUTOGEN_LINE).expect("banner");
