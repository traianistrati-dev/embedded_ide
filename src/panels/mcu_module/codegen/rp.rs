...
"
    ...
    /// A BUFFERED UART, cross-compiled.
    ///
    /// %TEMP%\eide_rp2040_buffered_check
    ///
    /// The unit tests above check the emitted TEXT, thoroughly - which channel
    /// is claimed, which handler is bound, what the card is told. None of them
    /// compiles it, and `BufferedUart` is an API this backend had never called:
    /// its argument order is embassy-rp's own, its buffers must outlive the
    /// call, and `new` returns `Self` where the DMA twin returns `Self` too but
    /// the STM32 template it was copied from returns a `Result`.
    #[test]
    #[ignore]
    fn emit_rp_buffered_project() {
        use crate::panels::mcu_module::modules::model::{ModuleConfig, UsartMode};
        let def = builtins::builtin_definitions()
            .into_iter()
            .find(|d| d.id == "rp2040_pico")
            .expect("built-in Pico");
        ...
        for p in mcu.iter_all_pins_mut() {
            match p.name.as_str() {
                "GP0" => p.selected_function = PinFunction::UsartTx(0),
                "GP1" => p.selected_function = PinFunction::UsartRx(0),
                // An SPI beside it, so the channel renumbering that the buffered
                // UART causes is compiled too, not just asserted.
                "GP18" => p.selected_function = PinFunction::SpiSck(0),
                "GP19" => p.selected_function = PinFunction::SpiMosi(0),
                "GP16" => p.selected_function = PinFunction::SpiMiso(0),
                _ => {}
        ...
        let main_rs = mcu.fresh_main_rs();
        assert!(
            main_rs.contains("BufferedUart::new("),
            "the harness built the right thing"
        );
...
"
        ...
        let dir = std::env::temp_dir().join("eide_rp2040_buffered_check");
        let _ = std::fs::remove_dir_all(&dir);
        project_gen::write_project(&dir, &files, &user, &mcu.mcu_config_text(), "")
            .expect("write rp buffered project");
        let toml_path = dir.join("Cargo.toml");
        let toml = std::fs::read_to_string(&toml_path).expect("read Cargo.toml");
        // `true` for the USART flag: the buffers are StaticCells, so this
        // project needs `static_cell` where the DMA one does not.
        let toml = project_gen::ensure_async_deps(
            &toml,
            true,
            project_gen::async_flavor_for(&mcu.family, ""),
